...
class BasicPipeline:
    ...
    def evaluate(self, dataset, do_eval=True, pred_process_fun=None, return_metric_dict=False):
...
class AdaptivePipeline(BasicPipeline):
    def __init__(
        ...
        generator = generator #get_generator(config)
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        wandb.init(project="rewriter", name='AdaptiveRAG', config=vars(self.config))
