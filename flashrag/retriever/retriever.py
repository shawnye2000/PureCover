...
class BaseRetriever:
    ...
    # @cache_manager
    # @rerank_manager
    # def hyde_search(self, *args, **kwargs):
    #     return self._hyde_search(*args, **kwargs)
...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(self, config: dict):
        ...
        if config["faiss_gpu"]:
            ...
            # self.index = faiss.index_cpu_to_gpu(res, 1, self.index)
            # self.index = faiss.index_cpu_to_gpus_list(self.index, gpus=[0], gpu_resources=gpu_resources) ##faiss.index_cpu_to_gpus_list(res, 1, self.index, options)
            self.index = faiss.index_cpu_to_all_gpus(self.index, co=co, ngpu=2) #
        self.corpus = load_corpus(self.corpus_path)
        self.topk = config["retrieval_topk"]
    ...
    def hyde_search(self, query, fake_doc_list, num=None, return_score=False):
        if num is None:
            num = self.topk
        all_emb_c = []
        for c in [query] + fake_doc_list:
            c_emb = self.encoder.encode(c)
            all_emb_c.append(np.array(c_emb))
        all_emb_c = np.array(all_emb_c)
        avg_emb_c = np.mean(all_emb_c, axis=0)
        if avg_emb_c.shape == (1, 768):
            hyde_vector = avg_emb_c
        else:
            hyde_vector = avg_emb_c.reshape((1, len(avg_emb_c)))
        # print(f'num:{type(num)}  {num}')
        scores, idxs = self.index.search(hyde_vector, k=num)
        scores = scores.tolist()
        idxs = idxs[0]
        scores = scores[0]
    ...
    def _batch_search(self, query_list: List[str], num: int = None, return_score=False):
        ...
        for start_idx in trange(0, len(query_list), batch_size):
